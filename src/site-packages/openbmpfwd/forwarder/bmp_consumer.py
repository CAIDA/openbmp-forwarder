...
from openbmpfwd.forwarder.logger import init_mp_logger
...
class BMPConsumer(multiprocessing.Process):
    ...
    def process_collector_msg(self, m):
        ...
        for row in map_list:
            if len(row):
                try:
                    self.LOG.debug("collector: %s [%s]", row[MsgBusFields.ADMIN_ID.get_name()], row[MsgBusFields.ACTION.get_name()])

                    if row[MsgBusFields.ACTION.get_name()] in ('started', 'heartbeat', 'change'):

                        # Update collector hash/cache
                        if row[MsgBusFields.HASH.get_name()] not in self.COLLECTORS:
                            self.COLLECTORS[row[MsgBusFields.HASH.get_name()]] = {'admin_id': row[MsgBusFields.ADMIN_ID.get_name()]}

                    else:
                        self.COLLECTORS.pop(row[MsgBusFields.HASH.get_name()], None)
    ...
    def process_router_msg(self, m):
        ...
        for row in map_list:
            if len(row):
                try:
                    self.LOG.debug("router: [%s] %s %s", row[MsgBusFields.ACTION.get_name()],
                                   row[MsgBusFields.NAME.get_name()], row[MsgBusFields.IP_ADDRESS.get_name()])

                    if row[MsgBusFields.ACTION.get_name()] in ('first', 'init'):
                        # Update the router hash/cache
                        if row[MsgBusFields.HASH.get_name()] not in self.ROUTERS:
                            self.ROUTERS[row[MsgBusFields.HASH.get_name()]] = {'ip': row[MsgBusFields.IP_ADDRESS.get_name()],
                                                              'name': row[MsgBusFields.NAME.get_name()]}
                    # else:
    ...
    def process_bmp_raw_msg(self, m):
        ...
        try:
            bmp_msg_obj.COLLECTOR_ADMIN_ID = self.COLLECTORS[m.get_collector_hash_id()] #collector hash
            bmp_msg_obj.ROUTER_IP = self.ROUTERS[m.get_router_hash_id()]['ip'] #router ip
            bmp_msg_obj.ROUTER_NAME = self.ROUTERS[m.get_router_hash_id()]['name'] #router name
